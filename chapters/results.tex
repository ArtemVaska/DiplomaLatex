...
Для всех видов, имеющих ``консервативную кассету``, были построены вторичные структуры для интрон-содержащего транскрипта с выделением цветом ``кассетного`` интрона (предоставляются по запросу).
...
Нас заинтересовал 2-й найденный мотив, так как его начало очень похоже на предложенную авторами~\cite{cte_consensus} консенсусную последовательность для CTE из рисунка~\ref{fig:CTE_consensus}.

К сожалению, использование Tomtom для поиска найденных консервативных мотивов из ``кассетного`` интрона в базе данных мотивов не дало статистически значимых результатов.
